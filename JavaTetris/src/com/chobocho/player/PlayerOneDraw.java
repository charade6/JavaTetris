...
public class PlayerOneDraw implements IPlayerDraw {
    ...
    public void onDraw(Graphics g, int startX, int startY, int blockWidth, int blockHeight) {
        ...
        if (tetris.isPlayState()) {
            ...
            int nextBlockX = startX + blockWidth * (tetris.getWidth())-13;
            int nextBlockY = startY + blockHeight * 6;
            g.setColor(Color.black);
            g.fillRect(nextBlockX+25, nextBlockY-20, 100, 100);
            drawBlock(g, nextblock, nextBlockX, nextBlockY, blockWidth/2, blockHeight/2);
            
            g.setColor(Color.black);
            g.setFont(new Font("Century Gothic", Font.BOLD, 32));
            g.drawString("NEXT", nextBlockX+30, nextBlockY-30);
        }

        int scorePointX =  startX + blockWidth * (tetris.getWidth());
        int scorePointY =  startY + blockHeight * 12;
        
        
        g.setColor(Color.black);
        g.setFont(new Font("Century Gothic", Font.BOLD, 32));
        g.drawString("SCORE", scorePointX+8, scorePointY-30);
        g.setFont(new Font("Century Gothic", Font.BOLD, 22));
        g.drawString(String.valueOf(tetris.getScore()), scorePointX+10, scorePointY);
    }
